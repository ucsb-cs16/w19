...
```
Using the function definition from above, state whether each of the following lines of code results in a compilation error or not. Write CE (for compile-time error) or NE (for No Error) 
