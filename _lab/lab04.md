...
Copy the skeleton to your local lab04 repo using the following command, <strong>REMEMBER</strong> to change the directory name in the commands below to your own directory's name, in this lab we simply use lab04_agaucho as a example for your local git directory:

```
  cp /cs/faculty/ykk/cs16/labs/lab04/* ~/cs16/lab04_agaucho/
```
...
In Lecture, we have introduced the idea of separate compilation, where your C++ program may be divided among multiple source files. The following web page explains more about separate compilation, dividing your program up among multiple C++ and .h files, and using a Makefile: [Separate Compilation and Makefiles](https://foo.cs.ucsb.edu/16wiki/index.php/C%2B%2B:_Separate_Compilation_and_Makefiles). I strongly encourage you to read over it briefly before you proceed with the lab.

## Step 5: Writing isOdd(), isEven() and isPrime()<a name="step3"></a>

We will be looking at both, the separate compilation and C++ coding.
...
Take a look at the <b>Makefile</b> file to understand why this happened (specifically, find the `clean` target and examine it's command).

## Step 5b: make utilityTest

This command makes the executable for a main program, defined in `utilityTest.cpp`, which tests the functions defined in `utility.cpp`.
For functions defined in a file such as `utility.cpp` that has no <b>main()</b>, the function prototypes are defined in the file `utility.h`.
Look at the source code for both `utility.cpp` and `utility.h` to see what they contain.
Recall that a "stub" is place-holder code that allows an incomplete function to compile. It is designed to fail all the tests, although, *for a Boolean function, since there are only two possible values (true and false), **any stub value** you choose is going to pass at least some of the tests*.

Compile `utilityTest` executable by running the `make utilityTest` command:
...
This runs the `utilityTest` program that tests the three functions `isOdd`, `isEven` and `isPrime`. As we noted above, some of the tests will pass, even though the implementation of the three functions is totally bogus (i.e., hard coded to return `false` always).
...
*   edit utility.cpp using a text editor
*   `make utilityTest`
*   run utilityTest (e.g., `./utilityTest`)

Save working versions of your code on github using the commands:

```
git add *.cpp *.h
git commit -m "implemented utility function - <insert name of function()>"
git push origin master
```

You must only proceed with the rest of the lab **after** you have implemented (i.e., written code for) **all the utility functions and _pass_ the provided test cases**. This is because for other files that you will be editing later, you will NEED functions `isOdd`, `isEven` and `isPrime`. Once you get them working, you will be able to call them in other files and KNOW that they work properly. You will not have to repeat the function definition.

When all the tests for `utlityTest` pass, do a final push to github (repeat the above 3 git commands) and move on to the next step.

**DO NOT proceed to the next step until you finished the steps above.**

<br/>


<h2>Step 6: Reviewing the rest of the files and your tasks </h2>
...
|countEvens.cpp|REPLACE STUB WITH CORRECT CODE.|You must replace the code in this file with correct code that returns the number of even integers in each array passed in. You will want to `#include "utility.h"` at the top of your file to use your completed function from step 5.
|countEvensTest.cpp|NO MODIFICATIONS NEEDED|This tests the changes you made in *countEvens.cpp*.
|countPrimes.cpp|REPLACE STUB WITH CORRECT CODE.|You must replace the code in this file with correct code that returns the number of prime integers in each array passed in. Treat negative numbers, 0 and 1 as "not prime". You will want to `#include "utility.h"` at the top of your file to use your completed function from step 5.
|countPrimesTest.cpp|NO MODIFICATIONS NEEDED|This tests the changes you made in *countPrimes.cpp*.
...
|sumOfArray.cpp|REPLACE STUB WITH CORRECT CODE.|You must replace the code in this file with correct code that returns the sum of the integers in each array passed in.
...
You should see a lot of activity as programs are compiled. You then will have a lot of executables you can run. Here is a list. Run each one and see what happens.
...
|countEvensTest|<b>*YES*</b>|Run this, and you will see all the tests fail. YOU NEED TO FIX THE `countEvens` function and then get all these tests to pass. You will want to `#include "utility.h"` at the top of your file to use your completed functions.
...
So, if you go through this list, and do all the things indicated, you are finished with the lab and ready to submit.
...
* Navigate to your lab04 directory, the one containing your code for this week's lab.
<pre>
...
Submit the **modified** cpp files to lab04 assignment on Gradescope. Then check that you have a correct score. If you did not receive the full score, modify your code to fix the issue(s) and re-submit the files again.


* You must check that you have followed these style guidelines:

1. Indentation is neat, consistent and follows good practice.  Code that is inside braces should be indented, and code that is at the same "level" of nesting inside braces should be indented in a consistent way.    Follow the examples from lecture, the sample code, and from the textbook. 
